...
from sabre.units.biostimulant_evaporator import BiostimulantEvaporator
...
def create_ad_biogas_system(
    ...
    PR = Press(
        ...
        cake_solids_wt_frac=(
            float(press_cake_solids_wt_frac)
            if press_cake_solids_wt_frac is not None
    ...
    # default if no side processing is used
    pressate_product = PR - 1
    ...
    if pb.get("enabled", False) and pb.get("concentrate_pressate", False):
        ...
        pressate_product = PC - 0

        # Optional finishing evaporator for a more realistic liquid biostimulant product
    ...
    selected_pretreatment = pretreatment_case or adS.get(
        "pretreatment_case", "press_mill_only"
    )
    pt_case = pretreatments.get(selected_pretreatment, {})
    pt_kind = pt_case.get("kind", "none")

    # Baseline AD values — per-case overrides take priority over global defaults
    vs_destruction = float(adp.get("vs_destruction", 0.20))

    ad_effects = pt_case.get("ad_effects", {})
    ch4_kg_per_kg_vs_fed = float(
        ad_effects.get("ch4_kg_per_kg_vs_fed",
            adp.get("ch4_kg_per_kg_vs_fed", 0.09))
    )

    # Per-case biogas composition (Chikani-Cabrera et al. 2022)
    ...
    if pt_kind == "none":
        ...
        HT = HeatingPretreatment(
            ...
            maintenance_frac_of_capex_per_yr=hA.get(
                "maintenance_frac_of_capex_per_yr", 0.035
            ),
        )
        ad_feed = HT - 0
        ...
        EZ = EnzymaticPretreatment(
            ...
            maintenance_frac_of_capex_per_yr=eA.get(
                "maintenance_frac_of_capex_per_yr", 0.035
            ),
        )
        ad_feed = EZ - 0
        ...
        PX = PeroxidePretreatment(
            ...
            maintenance_frac_of_capex_per_yr=pA.get(
                "maintenance_frac_of_capex_per_yr", 0.035
            ),
        )
        ad_feed = PX - 0
        ...
        EZ = EnzymaticPretreatment(
            "EZ",
            ins=PX - 0,
            outs=("combined_PE_treated_biomass",),
            temperature_K=eA.get("temperature_K", 308.15),
            residence_time_hr=eA.get("residence_time_hr", 24.0),
            enzyme_dose_kg_per_kg_dry_feed=eA.get("enzyme_dose_kg_per_kg_dry_feed", 0.02),
            treated_fraction=eA.get("treated_fraction", 1.0),
        ...
        EZ = EnzymaticPretreatment(
            "EZ",
            ins=HT - 0,
            outs=("combined_PTE_treated_biomass",),
            temperature_K=eA.get("temperature_K", 308.15),
            residence_time_hr=eA.get("residence_time_hr", 24.0),
            enzyme_dose_kg_per_kg_dry_feed=eA.get(
    ...
    if enable_feed_dilution:
        ...
        MX = bst.Mixer(
            "MX",
            ins=(ad_feed, dilution_water),
            outs=("diluted_ad_feed",),
        )
        ...
        def _set_dilution_water():
            ...
            if dry_mass <= 0.0:
                ...
                target_water_mass = dry_mass * target_feed_moisture_frac / (
                    1.0 - target_feed_moisture_frac
                )
    ...
    # -------------------------
    # H2S removal (iron sponge desulfurization)
    # -------------------------
    ...
    UP = BiogasUpgrading(
        "UP",
        ins=H2SR - 0,   # now takes treated (H2S-free) biogas
        ...
        maintenance_frac_of_capex_per_yr=upA.get(
            "maintenance_frac_of_capex_per_yr", 0.035
        ),
    )
    ...
    if EV is not None:
        path.append(EV)
    path.append(ML)
    path.extend(pt_units)
    if MX is not None:
        path.append(MX)
    path.extend([AD, H2SR, UP, SP])
